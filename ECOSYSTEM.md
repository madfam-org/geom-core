...
Zero-dependency C++17 core library for 3D geometry analysis: binary STL + STEP file parsing (OCCT optional), mesh analysis (volume, watertight, bounding box), printability checks, toolpath generation primitives. Consumed by yantra4d, digifab-quoting, blueprint-harvester, and pravara-mes via Python bindings or WASM.

**Pillar**: Fabrication / Geometry library
**Type**: library
**Status**: stable
**Registry product**: geom-core (`geom-core`, incubating).
...
# Logs
enclii logs (n/a — library) --env prod -f                    # live tail
enclii logs (n/a — library) --env prod --since 1h -n 200     # last hour
...
# Rollback
enclii rollback (n/a — library) --env prod                   # previous release
enclii rollback (n/a — library) v42 --env prod

# Releases + deployment history
enclii releases (n/a — library) -n 20
...
enclii domains list --service (n/a — library)
enclii domains add my.example.com --service (n/a — library)   # auto-provisions tunnel route + DNS
