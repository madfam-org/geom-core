# geom-core — Ecosystem Context

> **High-performance C++ geometry library with Python and WASM bindings for 3D printing.**
...
**Kubernetes namespace**: `(library — not deployed as a service)`
...
### Day-to-day for (n/a — library)

The commands below default to `(n/a — library)` — the primary service name for
...
enclii ps --wide
enclii ps (n/a — library) --env production

# Logs (tail, filter, history)
enclii logs (n/a — library) -f                          # live tail
enclii logs (n/a — library) --since 1h --level error    # last hour, errors only
enclii logs (n/a — library) --env staging -f
...
# Rollback
enclii rollback (n/a — library)                         # previous release
enclii rollback (n/a — library) --to-revision 5

# Releases + history
enclii releases (n/a — library)                          # list builds
enclii releases (n/a — library) --latest --output json

# Secrets (routed through Lockbox → Vault → ESO → K8s)
enclii secrets list (n/a — library)
enclii secrets set MY_KEY=value --service (n/a — library) --secret
enclii secrets rm MY_KEY --service (n/a — library)

# Domains, tunnel routes, DNS
enclii domains list (n/a — library)
enclii domains add (n/a — library) my.example.com       # auto-provisions tunnel route + DNS
...
# Routing (ingress + TLS)
enclii junctions list (n/a — library)
